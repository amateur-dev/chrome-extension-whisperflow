...
// Force single-threaded execution to avoid 'blob:' worker issues in extensions
env.backends.onnx.wasm.numThreads = 1;
env.backends.onnx.wasm.proxy = false;

// Point to local WASM files (copied to lib/) to avoid CDN fetches (blocked by CSP/MV3)
...
// Model configuration
// Using Whisper Tiny (multilingual) as it's more stable/standard than Moonshine
...
async function loadModel() {
  ...
  if (loadingPromise) {
    console.log('[WORKER] Joining existing model load...');
    try { await loadingPromise; } catch (e) { /* ignore */ }
    return;
  }
        ...
        progress_callback: (progress) => {
          ...
          if (progress.status === 'progress') {
            ...
            self.postMessage({
              ...
              message: `Downloading: ${progress.file?.split('/').pop() || 'model files'} (${percent}%)`,
              file: progress.file
            });
          } else if (progress.status === 'done') {
            console.log('[WORKER] File downloaded:', progress.file);
            self.postMessage({
              type: 'LOADING_PROGRESS',
              progress: 80,
              message: 'Processed ' + (progress.file?.split('/').pop() || 'file')
            });
...
async function transcribe(audioData, sampleRate = 16000, alreadyDecoded = false) {
  ...
  try {
    ...
    if (!transcriber) {
      if (loadingError) {
        throw new Error(`Model load failed: ${loadingError.message || loadingError}`);
      }
      throw new Error('Model not loaded properly (Unknown reason)');
    }
